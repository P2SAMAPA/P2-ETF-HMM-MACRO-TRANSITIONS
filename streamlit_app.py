...
st.set_page_config(page_title="HMM with Macro‑Driven Transitions", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🧠 HMM with Macro‑Driven Transitions</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Hidden Markov model where regime switching probabilities depend on macro variables</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 HMM")
...
st.sidebar.markdown(f"**Number of regimes:** {config.N_STATES} | **EM iterations:** {config.MAX_ITER}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'hmm_macro_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Regime prob: {etf['hmm_score_norm']:.3f}</p>
...
with tab1:
    st.header("🧠 Top ETFs by Probability of Being in Best Regime (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Hidden Markov Model (HMM)** with **macro‑driven transition probabilities**.
        - Regime switches depend on macro variables (VIX, DXY, yields) via multinomial logistic regression.
        - The score is the filtered probability (at the last time step) of being in the regime with highest mean return.
        - High probability → ETF is likely in a favourable regime given recent macro conditions.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the HMM probabilities.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("HMM with Macro‑Driven Transitions | Regime‑switching with macro covariates")
